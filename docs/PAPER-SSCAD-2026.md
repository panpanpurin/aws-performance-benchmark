...
| Base images | Four distinct bases across six Dockerfiles, every `FROM` pinned by `sha256:` digest, not by tag |
| Load generator | `artillery_version` and `form_data_version` pin what the instance installs; `benchmarks/suites/*/artillery/package-lock.json` pins the same versions for workstation runs, which use `npm ci` |
...
- ~~**Transitive Python dependencies are not locked yet.**~~ Closed.
  ...
  pinned, and so is the load generator: `form-data` builds the multipart body
  for the CSV and Thumbnail requests, so it is an input to the measurement
  rather than tooling, and it is pinned by `form_data_version` on the generator
  and by a committed lockfile for workstation runs. Dependency drift is no
  longer a threat to validity on either side of the request.
