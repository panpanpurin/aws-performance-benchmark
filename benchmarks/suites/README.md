...
| `prometheus.yml` | Scrape targets (app jobs are `REPLACE_ME` until synced) |
| `grafana/dashboard.json` | Suite dashboard |
| `artillery/` | Load tests (`test-ec2\|ecs\|lambda.yml`) + runners |
| `artillery/pilot-*.yml` | Generated by `make pilot-configs` from the committed `test-*.yml`: same target and headers, a shorter schedule below saturation. Do not hand-edit |
| `artillery/logs/` | Reports, run manifests and captured `app_*` metrics |
